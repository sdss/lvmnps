...
def str2action(s: str) -> Netio.ACTION:
    ...
    try:
        ...
        try:
            ...
            raise argparse.ArgumentTypeError(f"{s!r} is not a valid "
                                             f"{Netio.ACTION.__name__}")
...
def get_ids(id_strs: List[str], num_outputs: int) -> List[int]:
    ...
    for id_str in id_strs:
        if id_str.lower() == 'all':
            ...
            raise NetioException(f"Invalid output ID '{id_str}', "
...
def get_output_actions(ids_and_actions, num_outputs):
    ...
    if len(ids_and_actions) % 2 != 0:
        raise NetioException("Expecting ID ACTION pairs but got an "
    ...
    for pair in list(pairs):
        ...
        if id_str.lower() == "all":
            ...
            id = int(id_str)
            if id in result:
                raise NetioException("Multiple actions given for id "
                                     "'{}' but expecting only one".format(id))
            action = str2action(action_str)
            result[id] = action
        else:
            raise NetioException(f"Invalid output ID '{id_str}',"
                                 f"valid range is {1}-{max_id} or 'ALL'")
...
def load_config(args):
    ...
    config = configparser.ConfigParser({'user': '', 'password': '',
                                        'no_cert_warning': ''})
    if not args.conf:
    ...
    args.password = get_arg(args.password, config, 'password',
                            'NETIO_PASSWORD', u.netloc, None)
    args.no_cert_warning = get_arg(args.no_cert_warning, config,
                                   'no_cert_warning', None, u.netloc, False)

    # resolve the path of cert relative to configuration path
    basedir = os.path.dirname(args.conf) if args.conf else os.path.dirname(
                                                               os.path.curdir)
    args.cert = args.cert if isinstance(args.cert, bool) else os.path.join(basedir,
                                                                           args.cert)
...
def parse_args():
    ...
    parser.add_argument('device', metavar='DEVICE', action='store',
                        help='Netio device URL')

    parser.add_argument("-u", "--user", action="store", dest='user', metavar='U',
                        help='M2M API username')
    parser.add_argument("-p", "--password", action="store", dest='password',
                        metavar='P', help='M2M API password')

    parser.add_argument("-C", "--cert", action="store_false", dest='cert', default=True,
                        help='HTTPS Certificate')
    parser.add_argument("-c", "--config", action="store", dest="conf", metavar='CFG',
    ...
    command_parser = parser.add_subparsers(metavar="COMMAND",
                                           help="device command")
    ...
    get_parser = command_parser.add_parser("get", help="GET output state",
                                           aliases=['GET', 'G', 'g'])
    get_parser.add_argument('id', metavar='ID', nargs='*',
                            default=['ALL'], help='Output ID. All if not specified')
    get_parser.set_defaults(func=command_get)
    get_parser.add_argument("-d", "--delimiter", action="store",
                            dest="delim", default="\t", help='')
    get_parser.add_argument("--no-header", action="store_true",
                            help='don\'t print column description')
    get_parser.add_argument("--action-int", action="store_true",
                            help='print action as integer')
...
def main():
    ...
    try:
        ...
        device = Netio(urlunparse(u), auth_rw=(args.user, args.password),
                       verify=args.cert, skip_init=True)
...
def command_get(device: Netio, args: argparse.Namespace) -> None:
    ...
    if not args.no_header:
        print('id', 'State', 'Action', 'Delay', 'Current', 'PFactor',
              'Load', 'Energy', 'Name', sep=args.delim)
    for o in outputs:
        action = o.Action.name if not args.action_int else o.Action.value
        print(o.ID, o.State, action, o.Delay, o.Current,
              o.PowerFactor, o.Load, o.Energy, o.Name, sep=args.delim)
