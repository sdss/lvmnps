...
class PowerSwitchBase(object):
    ...
    def config_get(self, key, default=None):
        ...
        def getter(config, key, default=None):
            ...
            return default if c == None else c if len(k) == 1 else getter(c, k[1], default) if type(c) == dict else default
    ...
    async def setState(self, state, name: str = "", portnum: int = 0):
        if portnum > self.numports:
            return []
        return await self.switch(Outlet.parse(state), self.collectOutletsByNameAndPort(name, portnum))
